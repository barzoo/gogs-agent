...
`skills/gogs-agent/skill.md` 里定义了 16 个 tool，每个对应一条 CLI 命令，带 JSON Schema 描述参数。Claude 根据你的意图选 tool、填参数、跑 `gogs` 命令，拿到 JSON 结果后告诉你发生了什么。
...
2. **配置凭据：**
   ...
   export GOGS_BASE_URL=https://git.desiyi.com/api/v1   # 可选——这是默认值
